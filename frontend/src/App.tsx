...
function App() {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-slate-900 text-slate-100">
      <header className="px-6 py-3 bg-slate-800 border-b border-slate-700 flex items-center justify-between gap-4 shrink-0">
        <h1
          className="text-xl font-bold tracking-tight shrink-0 cursor-pointer"
          onClick={() => setView('map')}
        >
      ...
      <main className="flex-1 min-h-0 relative">
        {view === 'map' ? <MapView activeCategory={activeCategory} aiPins={aiPins} route={route} onRoute={setRoute} /> : <PricingPage />}
        {view === 'map' && aiOpen && (
